...
extern "C" void launch_softmax_oneflow(float* d_x, float* d_y, int rows, int cols) {
  using ComputeType = float;
  DirectLoad<float, ComputeType> load(d_x, cols);
  DirectStore<ComputeType, float> store(d_y, cols);
  cudaStream_t stream;
