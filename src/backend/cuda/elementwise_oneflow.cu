// https://github.com/BBuf/how-to-optim-algorithm-in-cuda/blob/master/elementwise/elementwise.cu
...
// 添加launch接口
extern "C" void launch_elementwise_add_oneflow(float* a, float* b, float* c, int n) {
    cudaStream_t stream;
    cudaStreamCreate(&stream);
    Binary(AddFunctor<float>(), n, c, a, b);
    cudaStreamSynchronize(stream);
    cudaStreamDestroy(stream);
}

extern "C" void launch_elementwise_relu_oneflow(float* a, float* b, int n) {
    cudaStream_t stream;
    cudaStreamCreate(&stream);
    Unary(ReluFunctor<float>(), n, b, a);
    cudaStreamSynchronize(stream);
    cudaStreamDestroy(stream);
}
