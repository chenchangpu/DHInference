...
void ModelLoader::load_build_model_from_file(graph_model* model, Tensor* input_tensor) {  // 输入数据：input_tensor
    ...
    // 输出tensor
    Tensor* layer_in_tensor = input_tensor;   //每层的输入tensor
    Tensor* output_tensor;  // 最后输出tensor
    for (int i = 0; i < model_config_.n_layers; ++i) {
        ...
        if(i == model_config_.n_layers - 1){
            model->set_result_tesnor(output_tensor);        // 设置model最后的输出result
        }
    }
...
Tensor* ModelLoader::loadLayerParameters(graph_model* model, Tensor* input, 
    ...
    float* residual1_ptr = (float*)gpu_malloc(size);
    ...
    // residual
    float* residual2_ptr = (float*)gpu_malloc(size);
...
Tensor* ModelLoader::loadAttentionParams(graph_model* model, Tensor* input, int input_dim, int hidden_dim) {
    ...
    try {
        if(model_backend_ == ModelBackend::CUDA){
            // 在move_data_to_gpu中不释放CPU内存
            graph_model_cuda::move_data_to_gpu(Wq_t, false);
            graph_model_cuda::move_data_to_gpu(Wk_t, false);
            graph_model_cuda::move_data_to_gpu(Wv_t, false);
            graph_model_cuda::move_data_to_gpu(Wo_t, false);
        }
        ...
        if(model_backend_ == ModelBackend::CUDA){
            ...
            float* Q_ptr = (float*)gpu_malloc(size);
            if (!Q_ptr) {
                throw std::runtime_error("Failed to allocate GPU memory for Q");
            }
            Tensor* Q_t = new Tensor(2, shape, Q_ptr);  Q_t->setisleaf(false);
            
            float* K_ptr = (float*)gpu_malloc(size);
            if (!K_ptr) {
                gpu_free(Q_ptr);
                throw std::runtime_error("Failed to allocate GPU memory for K");
            }
            Tensor* K_t = new Tensor(2, shape, K_ptr);  K_t->setisleaf(false);
            
            float* V_ptr = (float*)gpu_malloc(size);
            if (!V_ptr) {
                gpu_free(Q_ptr);
                gpu_free(K_ptr);
                throw std::runtime_error("Failed to allocate GPU memory for V");
            }
            Tensor* V_t = new Tensor(2, shape, V_ptr);  V_t->setisleaf(false);
        
            shape[0] = hidden_dim;
            shape[1] = input->shape(0);
            float* K_T_ptr = (float*)gpu_malloc(size);
            if (!K_T_ptr) {
                gpu_free(Q_ptr);
                gpu_free(K_ptr);
                gpu_free(V_ptr);
                throw std::runtime_error("Failed to allocate GPU memory for K_T");
            }
            Tensor* K_t_T = new Tensor(2, shape, K_T_ptr);  K_t_T->setisleaf(false);

            shape[0] = shape[1] = input->shape(0); // seq_len
            size = shape[0] * shape[1] * sizeof(float);
            float* QK_ptr = (float*)gpu_malloc(size);
            if (!QK_ptr) {
                gpu_free(Q_ptr);
                gpu_free(K_ptr);
                gpu_free(V_ptr);
                gpu_free(K_T_ptr);
                throw std::runtime_error("Failed to allocate GPU memory for QK");
            }
            Tensor* QK_t = new Tensor(2, shape, QK_ptr);    QK_t->setisleaf(false);
            QK_t->setscale(scale);          // QK有scale，需要设置

            float* P_ptr = (float*)gpu_malloc(size);
            if (!P_ptr) {
                gpu_free(Q_ptr);
                gpu_free(K_ptr);
                gpu_free(V_ptr);
                gpu_free(K_T_ptr);
                gpu_free(QK_ptr);
                throw std::runtime_error("Failed to allocate GPU memory for P");
            }
            Tensor* P_t = new Tensor(2, shape, P_ptr);      P_t->setisleaf(false);

            shape[0] = input->shape(0);
            shape[1] = hidden_dim;
            size = shape[0] * shape[1] * sizeof(float);
            float* PV_ptr = (float*)gpu_malloc(size);
            if (!PV_ptr) {
                gpu_free(Q_ptr);
                gpu_free(K_ptr);
                gpu_free(V_ptr);
                ...
                gpu_free(QK_ptr);
                gpu_free(P_ptr);
                throw std::runtime_error("Failed to allocate GPU memory for PV");
            }
            Tensor* PV_t = new Tensor(2, shape, PV_ptr);    PV_t->setisleaf(false);

            float* O_ptr = (float*)gpu_malloc(size);
            if (!O_ptr) {
                gpu_free(Q_ptr);
                gpu_free(K_ptr);
                gpu_free(V_ptr);
                ...
                gpu_free(P_ptr);
                gpu_free(PV_ptr);
                throw std::runtime_error("Failed to allocate GPU memory for O");
            }
            Tensor* O_t = new Tensor(2, shape, O_ptr);      O_t->setisleaf(false);
...
Tensor* ModelLoader::loadLayerNormParams(graph_model* model, Tensor* input,
    ...
    try {
        if(model_backend_ == ModelBackend::CUDA){
            // 在move_data_to_gpu中不释放CPU内存
            graph_model_cuda::move_data_to_gpu(gamma_t, false);
            graph_model_cuda::move_data_to_gpu(beta_t, false);
        }
        ...
        if(model_backend_ == ModelBackend::CUDA){
            ...
            float* output_ptr = (float*)gpu_malloc(out_size);
            if (!output_ptr) {
        ...
        // 清理资源
        if(model_backend_ == ModelBackend::CUDA){
            gpu_free(gamma_t->data());
            gpu_free(beta_t->data());
        }
        free(gamma_ptr);
        free(beta_ptr);
        delete gamma_t;
...
Tensor* ModelLoader::loadFFNParams(graph_model* model, Tensor* input, 
    ...
    try {
        if(model_backend_ == ModelBackend::CUDA){
            // 在move_data_to_gpu中不释放CPU内存
            graph_model_cuda::move_data_to_gpu(w1_t, false);
            graph_model_cuda::move_data_to_gpu(w2_t, false);
        }
        ...
        if(model_backend_ == ModelBackend::CUDA){
            ...
            size_t size = shape[0] * shape[1] * sizeof(float);
            float* o1_ptr = (float*)gpu_malloc(size);
            if (!o1_ptr) {
                throw std::runtime_error("Failed to allocate GPU memory for o1");
            }
            Tensor* o1_t = new Tensor(2, shape, o1_ptr);        // o1

            float* o2_ptr = (float*)gpu_malloc(size);
            if (!o2_ptr) {
                gpu_free(o1_ptr);
                throw std::runtime_error("Failed to allocate GPU memory for o2");
            }
